...
----> before:                 neighborTable.put(neighbor, new EdgeWrapper(1, networks[indicies.get(neighbor)]));
----> after:                 neighborTable.put(neighbor, null);
----> line number in original file: 422
----> mutated node: 8275
    ...
    private HashMap<Node, EdgeWrapper> createNeighbourTable(Graph graph, Node node, HashMap<Node, Integer> indicies,
        ...
        if (!directed) {
            ...
            for (Node neighbor : ((DirectedGraph) graph).getPredecessors(node)) {
                neighborTable.put(neighbor, null);
            }
