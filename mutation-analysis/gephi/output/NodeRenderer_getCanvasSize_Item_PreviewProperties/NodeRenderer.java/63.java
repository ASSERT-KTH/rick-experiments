...
----> after:     public void renderPDF(Item item, PDFTarget target, PreviewProperties properties) { item = null;
...
public class NodeRenderer implements Renderer {
    ...
    public void renderPDF(Item item, PDFTarget target, PreviewProperties properties) { item = null;
        Float x = item.getData(NodeItem.X);
        Float y = item.getData(NodeItem.Y);
        Float size = item.getData(NodeItem.SIZE);
        size /= 2f;
