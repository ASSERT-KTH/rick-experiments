...
----> before:         return createBuilder(Arrays.stream(flagEncodersStr.split(",")).filter(s -> !s.trim().isEmpty()).
----> after:         return createBuilder(Arrays.stream(flagEncodersStr.split(",")).filter(s ->  s.trim().isEmpty()).
